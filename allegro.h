...
#define ALLEGRO_VERSION_STR      "3.0 + WIP"
#define ALLEGRO_DATE_STR         "1998"
#define ALLEGRO_DATE             19980530    /* yyyymmdd */
...
void set_mouse_range(int _x1, int _y1, int _x2, int _y2);
...
void timer_simulate_retrace(int);

void rest(long _time);
void rest_callback(long _time, void (*callback)());
...
int keypressed();
int readkey();
void simulate_keypress(int _key);
void clear_keybuf();
...
#define joy_hat         ((joy[0].stick[1].axis[0].d1) ? 1 :             \
			   ((joy[0].stick[1].axis[0].d2) ? 3 :          \
			      ((joy[0].stick[1].axis[1].d1) ? 2 :       \
				 ((joy[0].stick[1].axis[1].d2) ? 4 :    \
				    0))))
...
#define GFX_VESA3             6
#define GFX_VBEAF             7
#define GFX_XTENDED           8
#define GFX_ATI               9
#define GFX_MACH64            10
#define GFX_CIRRUS64          11
#define GFX_CIRRUS54          12
#define GFX_PARADISE          13
#define GFX_S3                14
#define GFX_TRIDENT           15
#define GFX_ET3000            16
#define GFX_ET4000            17
#define GFX_ET6000            18
#define GFX_VIDEO7            19
...
extern GFX_DRIVER gfx_vesa_1, gfx_vesa_2b, gfx_vesa_2l, gfx_vesa_3,
		  gfx_vbeaf, gfx_xtended, gfx_ati, gfx_mach64, 
		  gfx_cirrus64, gfx_cirrus54, gfx_realtek, gfx_s3, 
		  gfx_trident, gfx_et3000, gfx_et4000, gfx_et6000, 
		  gfx_paradise, gfx_video7;
...
typedef struct GFX_VTABLE        /* functions for drawing onto bitmaps */
{
   ...
   void (*vline)(struct BITMAP *bmp, int x, int _y1, int _y2, int color);
   void (*hline)(struct BITMAP *bmp, int _x1, int y, int _x2, int color);
   void (*line)(struct BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int color);
   void (*rectfill)(struct BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int color);
   int  (*triangle)(struct BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int x3, int y3, int color);
...
__INLINE__ void _set_color(int _index, RGB *p)
{
   outportb(0x3C8, _index);
   outportb(0x3C9, p->r);
   outportb(0x3C9, p->g);
   outportb(0x3C9, p->b);

   _current_pallete[_index] = *p;
}
...
void set_clip(BITMAP *bitmap, int _x1, int _y1, int _x2, int _y2);
...
__INLINE__ void vline(BITMAP *bmp, int x, int _y1, int _y2, int color) 
{ 
   bmp->vtable->vline(bmp, x, _y1, _y2, color); 
}


__INLINE__ void hline(BITMAP *bmp, int _x1, int y, int _x2, int color) 
{ 
   bmp->vtable->hline(bmp, _x1, y, _x2, color); 
}


__INLINE__ void line(BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int color)
{
   bmp->vtable->line(bmp, _x1, _y1, _x2, _y2, color);
}


__INLINE__ void rectfill(BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int color)
{
   bmp->vtable->rectfill(bmp, _x1, _y1, _x2, _y2, color);
}
...
void do_line(BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int d, void (*proc)(BITMAP *, int, int, int));
void triangle(BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int x3, int y3, int color);
void polygon(BITMAP *bmp, int vertices, int *points, int color);
void rect(BITMAP *bmp, int _x1, int _y1, int _x2, int _y2, int color);
...
typedef struct RLE_SPRITE           /* a RLE compressed sprite */
{
   ...
   signed char dat[1];              /* RLE bitmap data */
} RLE_SPRITE;
...
__INLINE__ unsigned long bmp_write_line(BITMAP *bmp, int _line)
{
...
__INLINE__ unsigned long bmp_read_line(BITMAP *bmp, int _line)
{
   unsigned long result;
...
#ifdef DJGPP
/* for djgpp */
extern DIGI_DRIVER digi_sb, digi_gus, digi_audiodrive;
#else
/* for linux */
...
int install_sound(int _digi_card, int _midi_card, char *cfg_path);
void remove_sound();

int install_sound_input(int _digi_card, int _midi_card);
...
void voice_ramp_volume(int voice, int _time, int endvol);
void voice_stop_volumeramp(int voice);

int voice_get_frequency(int voice);
void voice_set_frequency(int voice, int frequency);
void voice_sweep_frequency(int voice, int _time, int endfreq);
void voice_stop_frequency_sweep(int voice);

int voice_get_pan(int voice);
void voice_set_pan(int voice, int pan);
void voice_sweep_pan(int voice, int _time, int endpan);
void voice_stop_pan_sweep(int voice);

void voice_set_echo(int voice, int strength, int _delay);
...
void cross_product(fixed _x1, fixed _y1, fixed z1, fixed x2, fixed y2, fixed z2, fixed *xout, fixed *yout, fixed *zout);
void cross_product_f(float _x1, float _y1, float z1, float _x2, float _y2, float z2, float *xout, float *yout, float *zout);

fixed polygon_z_normal(V3D *v1, V3D *v2, V3D *v3);
float polygon_z_normal_f(V3D_f *v1, V3D_f *v2, V3D_f *v3);


__INLINE__ fixed dot_product(fixed x1, fixed _y1, fixed z1, fixed x2, fixed y2, fixed z2)
{
   return fmul(x1, x2) + fmul(_y1, y2) + fmul(z1, z2);
}


__INLINE__ float dot_product_f(float _x1, float _y1, float z1, float _x2, float _y2, float z2)
{
   return (_x1 * _x2) + (_y1 * _y2) + (z1 * z2);
}
