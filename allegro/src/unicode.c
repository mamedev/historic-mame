...
/* this default table reduces Latin 1 characters to similar 7 bit ones */
 ...
 */
static int sprint_i(char **buf, double val, int precision)
{
   char tmp[512];
   int i = 0;
   int len;
...
/* sprint_float:
 *  Helper for formatting a float.
 */
static int sprint_float(char **buf, SPRINT_INFO *info, double val)
{
   int len = 0;
   int v, p, i;
   double int_port, frac_port;
   ...
   frac_port = modf(val, &int_port);

   len += sprint_i(buf, int_port, -1);

   if (info->precision >= 0)
      p = info->precision;
   else
      p = 8 - len + info->num_special;
	 ...
	 if (ugetc(format) == '%') {
	    ...
	    switch (c) {
		  ...
		  if (info.flags & SPRINT_FLAG_LONG_DOUBLE)
		     slen = sprint_float(&buf, &info, va_arg(args, long double));
		  else
		     slen = sprint_float(&buf, &info, va_arg(args, double));
		  NEXT_C();
		  break;
