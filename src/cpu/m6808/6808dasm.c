...
int Dasm6808 (char *buf, int pc)
{
	...
	if (*p != '_')
	{
		if (*++p != '_')
		{
			...
			{
				sprintf (buf, "%-5s$%04x", opstr, (pc + 2 + (signed char)cpu_readop_arg(pc+1)) & 0xffff);
				return 2;
			}
			else
			{
	...
	*p++ = 0;
	if (*p == 'i')
	{
		if (*++p == 'x')
		{
			if ( s[-1] == 'i' && s[0] == 'm' ) { /* EHC 04-11 Added AIM/OIM/EIM/TIM support */
				sprintf (buf, "%-5s#$%02x,(x+$%02x)", opstr, cpu_readop_arg(pc+1), cpu_readop_arg(pc+2) );
				return 3;
			}

			sprintf (buf, "%-5s(x+$%02x)", opstr, cpu_readop_arg(pc+1));
			return 2;
		}
		else
		{
			if (*s == 'x' || *s == 's' || *s == 'd')
			{
				sprintf (buf, "%-5s#$%04x", opstr, (cpu_readop_arg(pc+1) << 8) + cpu_readop_arg(pc+2));
				return 3;
			}
			else
			{
				sprintf (buf, "%-5s#$%02x", opstr, cpu_readop_arg(pc+1));
				return 2;
			}
		}
	}
	...
	{
		if ( s[-1] == 'i' && s[0] == 'm' ) { /* EHC 04-11 Added AIM/OIM/EIM/TIM support */
			sprintf (buf, "%-5s#$%02x,$%02x", opstr, cpu_readop_arg(pc+1), cpu_readop_arg(pc+2) );
			return 3;
		}
