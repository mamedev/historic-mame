...
#define INDEXED {eaddr=xreg+(UINT8)M_RDOP_ARG(pcreg++);}
...
{
#if NEW_INTERRUPT_SYSTEM
	if (pending_interrupts & M6808_PENDING) {
...
void m6808_set_irq_line(int irqline, int state)
{
	irq_state = state;
	if (state == CLEAR_LINE)
		pending_interrupts &= ~M6808_PENDING;
	else
		pending_interrupts |= M6808_PENDING;
}
