...
#define GetMemB(Seg,Off) (cycle_count-=6,(BYTE)cpu_readmem20((DefaultBase(Seg)+(Off))&i86_AddrMask))
#define GetMemW(Seg,Off) (cycle_count-=10,(WORD)GetMemB(Seg,Off)+(WORD)(GetMemB(Seg,(Off)+1)<<8))
#define PutMemB(Seg,Off,x) { cycle_count-=7; cpu_writemem20((DefaultBase(Seg)+(Off))&i86_AddrMask,(x)); }
#define PutMemW(Seg,Off,x) { cycle_count-=11; PutMemB(Seg,Off,(BYTE)(x)); PutMemB(Seg,(Off)+1,(BYTE)((x)>>8)); }

#define ReadByte(ea) (cycle_count-=6,(BYTE)cpu_readmem20((ea)&i86_AddrMask))
#define ReadWord(ea) (cycle_count-=10,cpu_readmem20((ea)&i86_AddrMask)+(cpu_readmem20(((ea)+1)&i86_AddrMask)<<8))
#define WriteByte(ea,val) { cycle_count-=7; cpu_writemem20((ea)&i86_AddrMask,val); }
#define WriteWord(ea,val) { cycle_count-=11; cpu_writemem20((ea)&i86_AddrMask,(BYTE)(val)); cpu_writemem20(((ea)+1)&i86_AddrMask,(val)>>8); }
