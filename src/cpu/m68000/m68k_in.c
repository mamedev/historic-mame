...
void m68020_bfchg(void)
{
   ...
   uint width = (((BIT_5(word2) ? CPU_D[word2&7] : word2) -1 ) & 31) + 1;
   uint data = ( (m68ki_read_32(base) << offset) | ((offset + width > 32) ? m68ki_read_8(base+4) >> (8-offset) : 0) ) >> (32-width);
   uint mask = MASK_OUT_ABOVE_32(0xffffffff << (32-width));
...
void m68000_divs_d_16(void)
{
   ...
   m68ki_exception(EXCEPTION_ZERO_DIVIDE);
}
...
void m68000_illegal(void)
{
   ...
   M68K_DO_LOG(( M68K_LOG, "%s at %08x: illegal instruction %04x (%s)\n",
                 m68k_cpu_names[CPU_MODE], ADDRESS_68K(CPU_PC-m68k_pc_offset), CPU_IR,
                 m68k_disassemble_quick(ADDRESS_68K(CPU_PC-m68k_pc_offset)) ));
#ifdef M68K_LOG
...
void m68000_movem_pd_16(void)
{
   ...
   for(;i<16;i++)
      ...
      {
         m68ki_write_16(ea -= 2, *(m68k_movem_pd_table[i]));
         count++;
      }
   AY = ea;
   USE_CLKS((count<<2)+8);
...
void m68000_movem_pd_32(void)
{
   ...
   for(;i<16;i++)
      if(register_list & (1 << i))
      {
         m68ki_write_32(ea -= 4, *(m68k_movem_pd_table[i]));
         count++;
      }
   AY = ea;
   USE_CLKS((count<<4)+8);
...
{
   m68ki_exception(EXCEPTION_TRAP_BASE+(CPU_IR&0xf));
}
...
void m68020_trapt_0(void)
{
   if(CPU_MODE & CPU_MODE_020_PLUS)
   {
      m68ki_exception(EXCEPTION_TRAPV);
      return;
   }
   m68000_illegal();
}
