...
#define PRINT_TABLE_ENTRY(file, name, mask, match, clks) fprintf(file, "   {%-24s, 0x%04x, 0x%04x},\n", name, mask, match)
...
/*  name                       mask   match    ea   size  base clks */
   ...
   {"m68000_movem_re_16"    , 0xffc0, 0x4880, 0x2f8, 1,  36},
   {"m68000_movem_re_32"    , 0xffc0, 0x48c0, 0x2f8, 2,  68},
   {"m68000_movem_er_16"    , 0xffc0, 0x4c80, 0x37b, 1,  40},
   {"m68000_movem_er_32"    , 0xffc0, 0x4cc0, 0x37b, 2,  72},
...
void add_table_footer(FILE* filep)
{
   ...
   fprintf(filep, "static int compare_nof_true_bits(const void* aptr, const void* bptr)\n");
   fprintf(filep, "{\n");
   fprintf(filep, "   uint a = ((opcode_handler_struct*)aptr)->mask;\n");
   fprintf(filep, "   uint b = ((opcode_handler_struct*)bptr)->mask;\n\n");

   fprintf(filep, "   a = ((a & 0xAAAA) >> 1) + (a & 0x5555);\n");
   fprintf(filep, "   a = ((a & 0xCCCC) >> 2) + (a & 0x3333);\n");
   fprintf(filep, "   a = ((a & 0xF0F0) >> 4) + (a & 0x0F0F);\n");
   fprintf(filep, "   a = ((a & 0xFF00) >> 8) + (a & 0x00FF);\n\n");

   fprintf(filep, "   b = ((b & 0xAAAA) >> 1) + (b & 0x5555);\n");
   fprintf(filep, "   b = ((b & 0xCCCC) >> 2) + (b & 0x3333);\n");
   fprintf(filep, "   b = ((b & 0xF0F0) >> 4) + (b & 0x0F0F);\n");
   fprintf(filep, "   b = ((b & 0xFF00) >> 8) + (b & 0x00FF);\n\n");

   fprintf(filep, "   return b - a; /* reversed to get greatest to least sorting */\n");
   fprintf(filep, "}\n\n");
...
int generate_funcs(FILE* file_ac, FILE* file_dm, FILE* file_nz)
{
   ...
   if((input_file=fopen("m68k_in.c", "rt")) == NULL)
   {
      perror("can't open m68k_in.c");
      exit(-1);
   }
   ...
   for(;;)
   {
      ...
      *(name_end+insert_pt) = 0;
      ...
      for(i=0;func_lines[ea_line][i] == ' ';i++)
         ea_spaces[i] = ' ';
      ea_spaces[i] = 0;
...
int main(void)
{
   ...
   if((prototype_file = fopen(prototype_filename, "wt")) == NULL)
   {
      fprintf(stderr, "Unable to create prototype file (%s)\n", prototype_filename);
      perror("");
      exit(-1);
   }
   if((table_file = fopen(table_filename, "wt")) == NULL)
   {
      fprintf(stderr, "Unable to create table file (%s)\n", table_filename);
      perror("");
      exit(-1);
   }
   if((ops_ac_file = fopen(ops_ac_filename, "wt")) == NULL)
   {
      fprintf(stderr, "Unable to create ops a-c file (%s)\n", ops_ac_filename);
      perror("");
      exit(-1);
   }
   if((ops_dm_file = fopen(ops_dm_filename, "wt")) == NULL)
   {
      fprintf(stderr, "Unable to create ops d-m file (%s)\n", ops_dm_filename);
      exit(-1);
   }
   if((ops_nz_file = fopen(ops_nz_filename, "wt")) == NULL)
   {
