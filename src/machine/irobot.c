...
static void scanline_callback(int i)
{
    i=0;
    if (irobot_scanline == 0) irvg_vblank=0;
    if (irobot_scanline == 224) irvg_vblank=1;
    /* set the IRQ line state based on the 32V line state */
    cpu_set_irq_line(0, M6809_IRQ_LINE, (irobot_scanline & 32) ? ASSERT_LINE : CLEAR_LINE);

    /* set a callback for the next 32-scanline increment */
    irobot_scanline += 32;
    if (irobot_scanline >= 256) irobot_scanline = 0;
    irvg_timer = timer_set(cpu_getscanlinetime(irobot_scanline), irobot_scanline, scanline_callback);
}
...
void irobot_init_machine (void) {
  ...
  if (irvg_timer) timer_remove(irvg_timer);

  /* set an initial timer to go off on scanline 0 */
  irvg_timer = timer_set(cpu_getscanlinetime(0), 0, scanline_callback);
