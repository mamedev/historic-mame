...
VIDEO_UPDATE( destroyr )
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		drawgfx(bitmap, Machine->gfx[2], num, 0, flipx, 0,
			horz, 16 * i, &Machine->visible_area[0], TRANSPARENCY_PEN, 0);
	}
	...
	for (i = 0; i < 8; i++)
	{
		for (j = 0; j < 32; j++)
		{
			...
			drawgfx(bitmap, Machine->gfx[0], num, 0, 0, 0,
				8 * j, 8 * i, &Machine->visible_area[0], TRANSPARENCY_PEN, 0);
		}
	}
	...
	for (i = 0; i < 2; i++)
	{
		...
		drawgfx(bitmap, Machine->gfx[1], destroyr_minor_obj_ram[i + 0], 0, 0, 0,
			horz, vert, &Machine->visible_area[0], TRANSPARENCY_PEN, 0);
	}
	...
	for (i = 0; i < 4; i++)
	{
		drawgfx(bitmap, Machine->gfx[3], destroyr_wavemod ? 1 : 0, 0, 0, 0,
			64 * i, 0x4e, &Machine->visible_area[0], TRANSPARENCY_PEN, 0);
	}
