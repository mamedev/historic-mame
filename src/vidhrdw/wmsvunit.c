...
data16_t *wmsvunit_videoram;
data32_t *wmsvunit_textureram;
...
{
	UINT16 *dest = &wmsvunit_videoram[(page_control & 4) ? 0x40000 : 0x00000];
	UINT16 pixdata = (dma_data[1] & 0xff00) | (dma_data[0] & 0x00ff);
...
{
	UINT16 *dest = &wmsvunit_videoram[(page_control & 4) ? 0x40000 : 0x00000];
	UINT8 *texbase = (UINT8 *)wmsvunit_textureram + (dma_data[14] * 256);
	UINT16 pixdata = dma_data[1] & 0xff00;
...
{
	COMBINE_DATA(&wmsvunit_videoram[offset]);
}


READ32_HANDLER( wmsvunit_videoram_r )
{
	return wmsvunit_videoram[offset];
}
...
{
	UINT8 *base = (UINT8 *)wmsvunit_textureram;
	base[offset * 2] = data;
...
VIDEO_UPDATE( wmsvunit )
{
	...
	for (y = cliprect->min_y; y <= cliprect->max_y; y++)
	{
		...
		for (x = 0; x < width; x++)
			*dest++ = wmsvunit_videoram[offset + x] & 0x7fff;
		offset += 512;
	}
}
