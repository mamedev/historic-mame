...
{
	int offset = (tile_index%32)*32+tile_index/32;
	unsigned char attributes = videoram2[offset];
	SET_TILE_INFO(gfxbank,videoram1[offset] + ((attributes & 0xc0) << 2),attributes & 0x07)
...
void gng_bgvideoram_w(int offset,int data)
{
	if (gng_bgvideoram[offset] != data)
	{
		...
		tilemap_mark_tile_dirty(bg_tilemap,(offset%32)*32+offset/32);
	}
}
