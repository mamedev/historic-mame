...
void momoko_draw_bg_pri(struct osd_bitmap *bitmap, int chr, int col, int x,int y,int pri)
{
	int xx,sx,sy,dot;
	data32_t gfxadr;
	data8_t d0, d1;
	...
	for (sy=0; sy<8; sy++)
	{
		...
		for (xx=0; xx<2; xx++)
		{
			...
			for (sx=0; sx<4; sx++)
			{
				...
				if (dot>=pri)
					plot_pixel(bitmap, x+sx+xx*4, y+sy, Machine->pens[col*16+dot+256]);
				d0 = d0 << 1;
				d1 = d1 << 1;
			}
		}
...
void momoko_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	int x, y, dx, dy, rx, ry, radr, chr, sy, fx, fy, offs, col, pri ;
	...
	if (momoko_bg_mask == 0)
	{
		for (y=0; y<32; y++)
		{
			for (x=0; x<32; x++)
			{
				...
				chr = BG_MAP[ radr ] ;
				col = BG_COL_MAP[ chr+momoko_bg_select*512 ] & 0x0f;
...
/* draw sprites (momoko) */
	...
	for (offs=0; offs<9*4; offs +=4)
	{
		...
		fx = spriteram[offs+2] & 0x10;
		fy = spriteram[offs+2] & 0x08; /* ??? */
		x = spriteram[offs+3];
		y = 255-spriteram[offs+0]-16;
		...
		for (y=0; y<32; y++)
		{
			for (x=0; x<32; x++)
			{
				...
				if (pri != 0)
				{
					col = col & 0x0f;
					chr = chr + momoko_bg_select * 512;
					momoko_draw_bg_pri(bitmap,chr,col,8*x+dx-6,8*y+dy+9,pri);
				}
			}
		}
	}
...
/* draw FG layer */
	...
	if (momoko_fg_mask==0)
	{
		...
		for (y=0; y<32; y++)
		{
			for (x=0; x<32; x++)
			{
				radr = ((ry + y +32) & 0x3f) *0x20 + ((rx + x) & 0x1f) + (momoko_fg_select & 3) *0x800;
 				chr = FG_MAP[ radr ] ;
