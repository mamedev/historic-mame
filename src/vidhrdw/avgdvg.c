		...
		switch (opcode)
		{
			...
			case 9:
				...
				if (translucency)
					...
					if (z) z = (z << 4) | 0x0f;
				vector_add_point(currentx, currenty, VECTOR_COLOR444(colorram[1]), z);
	...
	while (!done)
	{
		...
		switch (opcode)
		{
			case VCTR:
				...
				vector_add_point(currentx, currenty, VECTOR_COLOR444(colorram[color]), z);
...
{
	int i = (data & 1) ? 0x0f : 0x08;
	int r = (data & 2) ? 0x00 : i;
	int g = (data & 8) ? 0x00 : i;
	int b = (data & 4) ? 0x00 : i;

	colorram[offset & 0x0f] = (r << 8) | (g << 4) | b;
}
