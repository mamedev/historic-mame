...
struct MemoryWriteAddress williams_adpcm_writemem[] =
{
	...
	{ 0x3400, 0x3400, williams_adpcm_6295_bank_select_w }, /* PCM-BS */
...
{
	UINT8 *RAM = Machine->memory_region[Machine->drv->cpu[williams_cpunum].memory_region];
	int bank = data & 3;
...
{
	UINT8 *RAM = Machine->memory_region[Machine->drv->cpu[williams_cpunum + 1].memory_region];
	int bank = data & 7;
	return &RAM[0x10000 + (bank * 0x8000)];
}
...
void williams_cvsd_init(int cpunum, int pianum)
{
	...
	RAM = Machine->memory_region[Machine->drv->cpu[cpunum].memory_region];
...
void williams_adpcm_init(int cpunum)
{
	...
	switch (entry_point)
	{
		...
		case 0xdd55:
			...
			dac.state_bank	= install_mem_write_handler(cpunum, 0x23a, 0x23a, dac_state_bank_w);
			dac.address		= NULL;	/* in Y */
			dac.end			= &RAM[0x232];
...
void williams_narc_init(int cpunum)
{
	...
	/* install the fixed ROM */
	RAM = Machine->memory_region[Machine->drv->cpu[cpunum + 1].memory_region];
	memcpy(&RAM[0xc000], &RAM[0x4c000], 0x4000);
	RAM = Machine->memory_region[Machine->drv->cpu[cpunum].memory_region];
...
static void locate_audio_hotspot(UINT8 *base, UINT16 start)
{
	...
	for (i = start; i < 0x10000; i++)
	{
			...
			base[i +10] == 0x1c && base[i +11] == 0xaf)				/* 1C AF       ANDCC #$00AF  */
		{
			...
			if (errorlog) fprintf(errorlog, "Found hotspot @ %04X", i);
			return;
		}
	}
