		...
		case 7:
			switch (reg)
			{
				...
				case 2:
					PARAM_WORD (pm);
					if (pm & 0x8000)
						sprintf (buf, "(-$%X,PC) [%X]", -(signed short)pm & 0xffff, pc + (signed short)pm + 2);
					else
						sprintf (buf, "($%lX,PC) [%lX]", pm, pc + pm + 2);
					break;
				case 3:
					PARAM_WORD (pm);
					temp = pm & 0xff;
...
int Dasm68000 (unsigned char *pBase, char *buffer, int pc)
{
	...
	switch (op & 0xffc0)
	{
		case 0x0000:
			...
			{
				ea = MakeEA (lo, p, 1, &count, pc); p += count;
		...
		case 0x0040:
			...
			{
				ea = MakeEA (lo, p, 2, &count, pc); p += count;
				sprintf (buffer, "ORI.W    #$%lX,%s", pm & 0xffff, ea);
			}
			break;
		case 0x0080:
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count, pc); p += count;
		...
		case 0x0280:
			...
			break;
		case 0x0400:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 1, &count, pc); p += count;
			sprintf (buffer, "SUBI.B   #$%lX,%s", pm & 0xff, ea);
			break;
		case 0x0440:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count, pc); p += count;
		...
		case 0x0e40:
			...
			break;
		case 0x0e80:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 4, &count, pc); p += count;
			if (pm & 0x0800)
			{
				if (pm & 0x8000)
		...
		case 0x1e00: case 0x1e80: case 0x1ec0: case 0x1f00: case 0x1f40: case 0x1f80:
			ea = MakeEA (lo, p, 1, &count, pc); p += count; ea2 = MakeEA (((op >> 9) & 0x07) + ((op >> 3) & 0x38), p, 1, &count, pc); p += count;
		...
		case 0x2e00: case 0x2e80: case 0x2ec0: case 0x2f00: case 0x2f40: case 0x2f80:
			ea = MakeEA (lo, p, 4, &count, pc); p += count; ea2 = MakeEA (((op >> 9) & 0x07) + ((op >> 3) & 0x38), p, 4, &count, pc); p += count;
		...
		case 0x2040: case 0x2240: case 0x2440: case 0x2640: case 0x2840: case 0x2a40: case 0x2c40: case 0x2e40:
			ea = MakeEA (lo, p, 4, &count, pc); p += count;
		...
		case 0x3e00: case 0x3e80: case 0x3ec0: case 0x3f00: case 0x3f40: case 0x3f80:
			ea = MakeEA (lo, p, 2, &count, pc); p += count; ea2 = MakeEA (((op >> 9) & 0x07) + ((op >> 3) & 0x38), p, 2, &count, pc); p += count;
