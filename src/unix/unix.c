...
Colormap 	 		 		 colormap;
XImage 						*image;
GC 				 		 		 gc;						/* Just the default GC - we don't really draw. */
unsigned long  		 White,
							 		 Black;
unsigned long 		*xpixel = NULL;	/* Pixel index values for the colors we need. */

char 							*title 				= "MultiPac4X v0.1";
int 					 		 snapshot_no	=	0;
byte				  		*buffer_ptr;
...
#define AUDIO_MIX_BUFSIZE			(256)
#define AUDIO_NUM_VOICES			(6)
#define AUDIO_TIMER_FREQ			((10*(AUDIO_SAMPLE_FREQ+AUDIO_MIX_BUFSIZE))/(10*(AUDIO_MIX_BUFSIZE)))
	  ...
	  for (j = 0; j < AUDIO_MIX_BUFSIZE; j++)
		...
		for (i = 0; i < AUDIO_NUM_VOICES; i++)
		{
			if (audio_on[i])
			{
				for (j = 0; j < AUDIO_MIX_BUFSIZE; j++)
				{
   				intbuf[j] |= 32768 + (audio_vol[i]/2) * audio_data[i][((j * audio_freq[i]) / AUDIO_SAMPLE_FREQ) % 
   												audio_len[i]];
   			}
...
/*
    if (ioctl (audio_fd, SNDCTL_DSP_NONBLOCK)) {
      printf ("Can't set DSP non-blocking\n");
      exit (1);
    }
