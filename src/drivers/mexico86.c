...
//			if (gfx_attr & 0x40) sx -= 256;
		...
		for (xc = 0;xc < 2;xc++)
		{
			for (yc = 0;yc < height;yc++)
			{
				...
				code = videoram[goffs] + ((videoram[goffs + 1] & 0x07) << 8) +
						((videoram[goffs + 1] & 0x80) << 4) + (charbank << 12);
				color = ((videoram[goffs + 1] & 0x38) >> 3) + ((gfx_attr & 0x02) << 2);
...
static unsigned char *shared, *protection_ram;
...
static void mexico86_bankswitch_w(int offs,int data)
{
...
//if (errorlog) fprintf(errorlog,"%04x: 68705 port B write %02x\n",cpu_get_pc(),data);
	...
	{
		address = portA_out;
if (errorlog && address >= 0x80) fprintf(errorlog,"%04x: 68705 address %02x\n",cpu_get_pc(),portA_out);
	}
...
//if (errorlog) fprintf(errorlog,"%04x: 68705 write %02x to address %04x\n",cpu_get_pc(),portA_out,address);
				protection_ram[address] = portA_out;
		}
	}
	if ((ddrB & 0x20) && (~data & 0x20) && (portB_out & 0x20))
	{
		cpu_irq_line_vector_w(0,0,protection_ram[0]);
...
static struct MemoryReadAddress readmem[] =
{
	...
	{ 0xe800, 0xe87f, MRA_RAM },	/* protection ram */
	{ 0xe880, 0xefff, MRA_RAM },
	{ 0xf010, 0xf010, input_port_5_r },
	{ 0xf800, 0xffff, MRA_RAM },	/* communication ram - to connect 4 players's subboard */
	{ -1 }  /* end of table */
};

static void bingobongo(int offset,int data)
{
	/*
	  waving my hands like mad.
	  this could even be right. It allows to enter service mode, fixing a race
	  condition between the two cpus (the second reads dip switches, but then the
	  first clears the shared ram and reads it again before the second has had
	  the time to read the dips again).
	 */
	cpu_set_reset_line(1,(data & 2) ? CLEAR_LINE : ASSERT_LINE);
}
static void bangobengo(int offset,int data)
{
	/* One of the bits reset the mcu - necessary for Kick & Run */
	if ((data & 0x01) == 0)	/* ??? */
...
static struct MemoryWriteAddress writemem[] =
{
	...
	{ 0xc000, 0xcfff, MWA_RAM, &videoram },
	{ 0xd500, 0xd7ff, MWA_RAM, &mexico86_objectram, &mexico86_objectram_size },
	{ 0xe800, 0xe87f, MWA_RAM, &protection_ram },	/* protection ram */
	{ 0xe880, 0xefff, MWA_RAM },
	{ 0xf000, 0xf000, mexico86_bankswitch_w },	/* program and gfx ROM banks */
	{ 0xf008, 0xf008, bingobongo },
	{ 0xf018, 0xf018, bangobengo },
...
static struct MachineDriver machine_driver =
{
	/* basic machine hardware */
	{
		...
		{
			CPU_Z80,
			2000000,		/* 2 MHz??? (music tempo depends on this) */
			3,
...
ROM_START( mexico86_rom )
	...
	ROM_LOAD( "prom1.bin",  0x0000, 0x0100, 0xbe6eb1f0 )
	ROM_LOAD( "prom3.bin",  0x0100, 0x0100, 0x3e953444 )
	ROM_LOAD( "prom2.bin",  0x0200, 0x0100, 0x14f6c28d )
