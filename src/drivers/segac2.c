...
static UINT8		segac2_palbank;
static UINT8		segac2_bg_palbase;
static UINT8		segac2_sp_palbase;
...
void vdp_reload_counter(int scanline)
{
	...
	if ((segac2_vdp_regs[0] & 0x10) && !(misc_io_data[7] & 0x10))
...
void segac2_ym3438_interrupt(int state)
{
	ym3438_int = state;
	update_interrupts();
}
...
static READ16_HANDLER( palette_r )
{
	...
	return paletteram16[offset + segac2_palbank * 0x200];
}
...
static void recompute_palette_tables(void)
{
	...
	for (i = 0; i < 4; i++)
	{
		int bgpal = 0x000 + segac2_bg_palbase * 0x40 + i * 0x10;
		int sppal = 0x100 + segac2_sp_palbase * 0x40 + i * 0x10;

		if (!alt_palette_mode)
		{
			segac2_bg_pal_lookup[i] = segac2_palbank * 0x200 + bgpal;
			segac2_sp_pal_lookup[i] = segac2_palbank * 0x200 + sppal;
		}
		else
		{
			segac2_bg_pal_lookup[i] = segac2_palbank * 0x200 + ((bgpal << 1) & 0x180) + ((~bgpal >> 2) & 0x40) + (bgpal & 0x30);
			segac2_sp_pal_lookup[i] = segac2_palbank * 0x200 + ((~sppal << 2) & 0x100) + ((sppal << 2) & 0x80) + ((~sppal >> 2) & 0x40) + ((sppal >> 2) & 0x20) + (sppal & 0x10);
		}
	}
}
...
static WRITE16_HANDLER( control_w )
{
	...
	/* bit 1 resets the protection somehow */
...
static ADDRESS_MAP_START( main_map, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x800000, 0x8001ff) AM_MIRROR(0x13fc00) AM_READWRITE(prot_r, prot_w)
	AM_RANGE(0x800200, 0x8003ff) AM_MIRROR(0x13fc00) AM_WRITE(control_w)
	AM_RANGE(0x840000, 0x84001f) AM_MIRROR(0x100000) AM_READWRITE(io_chip_r, io_chip_w)
	AM_RANGE(0x840100, 0x840107) AM_MIRROR(0x100000) AM_READWRITE(ym3438_r, ym3438_w)
	AM_RANGE(0x880000, 0x880001) AM_MIRROR(0x100000) AM_WRITE(segac2_upd7759_w)
	AM_RANGE(0x880134, 0x880135) AM_MIRROR(0x100000) AM_WRITE(counter_timer_w)
	AM_RANGE(0x880334, 0x880335) AM_MIRROR(0x100000) AM_WRITE(counter_timer_w)
	AM_RANGE(0x8c0000, 0x8c0fff) AM_MIRROR(0x100000) AM_READWRITE(palette_r, palette_w) AM_BASE(&paletteram16)
	AM_RANGE(0xc00000, 0xc0000f) AM_MIRROR(0x18ff00) AM_WRITE(segac2_vdp_w)
	AM_RANGE(0xc00010, 0xc00017) AM_MIRROR(0x18ff00) AM_WRITE(sn76489_w)
	AM_RANGE(0xc00000, 0xc0001f) AM_MIRROR(0x18ff00) AM_READ(segac2_vdp_r)
	AM_RANGE(0xe00000, 0xe0ffff) AM_MIRROR(0x1f0000) AM_RAM AM_BASE(&main_ram)
ADDRESS_MAP_END
...
struct YM3438interface segac2_ym3438_intf =
{
	segac2_ym3438_interrupt				/* IRQ handler */
};
...
GAME ( 1991, twinsqua, 0,        segac2,   twinsqua, twinsqua, ROT0, "Sega",                   "Twin Squash" )
...
GAME ( 1992, tantrbl,  tantr,    segac2,   ichidant, c2boot,   ROT0, "bootleg",                "Tant-R (Puzzle & Action) (Japan) (bootleg set 1)" )
GAME ( 1994, tantrbl2, tantr,    segac,    ichidant, tantr,    ROT0, "bootleg",                "Tant-R (Puzzle & Action) (Japan) (bootleg set 2)" )
GAME ( 1992, puyo,     0,        segac2,   puyo,     puyo,     ROT0, "Sega / Compile",         "Puyo Puyo (World)" )
GAME ( 1992, puyobl,   puyo,     segac2,   puyo,     puyo,     ROT0, "bootleg",                "Puyo Puyo (World, bootleg)" )
GAME ( 1992, puyoj,    puyo,     segac2,   puyo,     puyo,     ROT0, "Sega / Compile",         "Puyo Puyo (Japan)" )
GAME ( 1992, puyoja,   puyo,     segac2,   puyo,     puyo,     ROT0, "Sega / Compile",         "Puyo Puyo (Japan, Rev A)" )
...
GAME ( 1994, ichidntb, ichidant, segac,    ichidant, c2boot,   ROT0, "bootleg",                "Ichidant-R (Puzzle & Action 2) (Japan) (bootleg)" )
...
GAMEX( 1995, pclubjv2, pclubj,   segac2,   pclub,    pclubjv2, ROT0, "Atlus",                  "Print Club (Japan Vol.2)", GAME_NOT_WORKING )
GAMEX( 1996, pclubjv4, pclubj,   segac2,   pclub,    pclubjv4, ROT0, "Atlus",                  "Print Club (Japan Vol.4)", GAME_NOT_WORKING )
GAMEX( 1996, pclubjv5, pclubj,   segac2,   pclub,    pclubjv5, ROT0, "Atlus",                  "Print Club (Japan Vol.5)", GAME_NOT_WORKING )
