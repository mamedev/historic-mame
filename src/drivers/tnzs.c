...
TO-DO: - Find out how the hardware credit-counter works (MPU)
       - Fix problem with sprites and tiles being a few pixels out when the
         screen is flipped.
       - Verify dip switches
       - Sprite flip Y is not supported correctly (almost never used, but it
	     is used in the "welcome to Heaven" animation). The 16x16 tiles are
		 correctly Y flipped, but their order should also be inverted.
