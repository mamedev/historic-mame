...
static WRITE_HANDLER( mrflea_main_w ){
	if( mrflea_status&0x01 )
		logerror("pc=0x%04x; bad main write: 0x%02x\n", cpu_get_pc(), data );
	mrflea_status |= 0x01; // pending command to main CPU
	mrflea_main = data;
}

static WRITE_HANDLER( mrflea_io_w ){
	if( mrflea_status&0x08 )
		logerror("pc=0x%04x; bad io write: 0x%02x\n", cpu_get_pc(), data );
...
static READ_HANDLER( mrflea_main_r ){
	if( (mrflea_status&0x01)==0 )
		logerror("pc=0x%04x; bad main read: 0x%02x\n", cpu_get_pc(), mrflea_main );
	mrflea_status &= ~0x01; // main CPU command read
	return mrflea_main;
}

static READ_HANDLER( mrflea_io_r ){
	if( (mrflea_status&0x08)==0 )
		logerror("pc=0x%04x; bad io read: 0x%02x\n", cpu_get_pc(), mrflea_io );
...
WRITE_HANDLER( mrflea_select0_w ){
	mrflea_select0 = data;
}

WRITE_HANDLER( mrflea_select1_w ){
	mrflea_select1 = data;
}

WRITE_HANDLER( mrflea_select2_w ){
	mrflea_select2 = data;
}

WRITE_HANDLER( mrflea_select3_w ){
	mrflea_select3 = data;
}
...
static READ_HANDLER( mrflea_input0_r ){
	...
	logerror( "unk input0_r(%d)\n", mrflea_select0 );
	return 0x00;
}

static READ_HANDLER( mrflea_input1_r ){
	logerror( "unk input1_r(%d)\n", mrflea_select1 );
	return 0x00;
}
...
static READ_HANDLER( mrflea_input2_r ){
	...
	logerror( "unk input2_r(%d)\n", mrflea_select2 );
	return 0x00;
}

static READ_HANDLER( mrflea_input3_r ){
	if( mrflea_select2 == 0x0f ) return readinputport(4);
	logerror( "unk input3_r(%d)\n", mrflea_select3 );
	return 0x00;
}

//	AY8910_control_port_0_w
//	AY8910_read_port_1_r
...
ROM_START( mrflea )
	...
	ROM_LOAD( "io_c11",	0x1000, 0x1000, 0x8d26e0c8 )
	ROM_LOAD( "io_d11",	0x2000, 0x1000, 0xabd9afc0 )
...
*/
	PORT_DIPNAME( 0x03, 0x03, "Unknown DSW2" )
	...
	PORT_DIPSETTING( 0x00, DEF_STR( On ) )
	PORT_DIPNAME( 0xf0, 0xf0, DEF_STR( Unused ) )
	PORT_DIPSETTING( 0xf0, DEF_STR( Off ) )
	PORT_DIPSETTING( 0x00, DEF_STR( On ) )

	PORT_START /* ? */
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN | IPF_4WAY | IPF_COCKTAIL )
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_JOYSTICK_UP | IPF_4WAY | IPF_COCKTAIL )
