...
static struct MemoryWriteAddress turbofrc_writemem[] =
{
	...
	{ 0x0fc000, 0x0fc7ff, aerofgt_spriteram_2_w, &spriteram_2 },	/* different from aero */
...
static struct MemoryWriteAddress aerofgt_writemem[] =
{
	...
	{ 0x1d0000, 0x1d1fff, aerofgt_spriteram_2_w, &spriteram_2 },
...
static struct GfxLayout turbofrc_spritelayout1 =
{
	16,16,	/* 16*16 sprites */
	16384,	/* 16384 sprites */
	4,	/* 4 bits per pixel */
	{ 0, 1, 2, 3 },
	{ 1*4, 0*4, 3*4, 2*4, 16384*64*8+1*4, 16384*64*8+0*4, 16384*64*8+3*4, 16384*64*8+2*4,
			5*4, 4*4, 7*4, 6*4, 16384*64*8+5*4, 16384*64*8+4*4, 16384*64*8+7*4, 16384*64*8+6*4 },
	{ 0*32, 1*32, 2*32, 3*32, 4*32, 5*32, 6*32, 7*32,
			8*32, 9*32, 10*32, 11*32, 12*32, 13*32, 14*32, 15*32 },
	64*8	/* every char takes 32 consecutive bytes */
};

static struct GfxLayout turbofrc_spritelayout2 =
{
	16,16,	/* 16*16 sprites */
	8192,	/* 8192 sprites */
	4,	/* 4 bits per pixel */
...
{
	{ 1, 0x000000, &turbofrc_charlayout,      0, 32 },
	{ 1, 0x100000, &turbofrc_spritelayout1, 512, 16 },
	{ 1, 0x300000, &turbofrc_spritelayout2, 768, 16 },
	{ -1 } /* end of array */
};
...
ROM_START( turbofrc_rom )
	...
	/* 0x080000-0x0fffff empty - is there a missing ROM here? */
	ROM_LOAD( "tfrcu116.bin", 0x100000, 0x080000, 0xdf210f3b )
	ROM_LOAD( "tfrc5.bin",    0x180000, 0x040000, 0xf61d1d79 )
	ROM_LOAD( "tfrcu117.bin", 0x200000, 0x080000, 0xf70812fd )
	ROM_LOAD( "tfrc4.bin",    0x280000, 0x040000, 0x474ea716 )
	ROM_LOAD( "tfrcu134.bin", 0x300000, 0x080000, 0x487330a2 )
	ROM_LOAD( "tfrcu135.bin", 0x380000, 0x080000, 0x3a7e5b6d )

	ROM_REGION(0x30000)	/* 64k for the audio CPU + banks */
	ROM_LOAD( "tfrc6.bin",    0x00000, 0x20000, 0x2ca14a65 )
