...
static void twin16_decode( void ){
	gfx_untangle();
	twin16_custom_vidhrdw = 0;
}

static void fround_decode( void ){
	gfx_untangle();
...
ROM_START( miaj )
	...
	ROM_REGION_DISPOSE( 0x4000 ) /* chars */
...
INPUT_PORTS_START( twin16_input_ports ) /* generic */
	PORT_START      /* 0xa0001 */
...
INPUT_PORTS_START( miaj_input_ports )
	PORT_START      /* 0xa0001 */
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_COIN1 )
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_COIN2 )
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNKNOWN )
...
extern struct GameDriver mia_driver;

struct GameDriver miaj_driver =  {
	__FILE__,
	&mia_driver,
	"miaj",
