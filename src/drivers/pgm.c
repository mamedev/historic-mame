...
//  return 0;
	...
	{
		//ErrorLogMessage("%06X: ASIC25 W CMD %X  VAL %X",M68000_regs.pc,kb_cmd,data);
		if(kb_cmd==0)
			reg=data;
		else if(kb_cmd==2)
		{
			...
			if(data==1)	//Execute cmd
			{
				...
				//printf("command %04x\n",cmd);
				if(cmd==0x6d)	//?? get the number of chars in the selection creen
				{
					killbld_sharedprotram[0x29c/2] = 0x0000;
					killbld_sharedprotram[0x29e/2] = 0x000c;
				}
				if(cmd==0x4f)	//memcpy
				{
				//  unsigned short src=fast_r16(0x300290);
				//  unsigned int dst=0x300000+fast_r16(0x300292)<<1;
				//  unsigned short size=fast_r16(0x300294);
				//  unsigned short mode=fast_r16(0x300296)&0xf;
				//  int a=1;
				//  if(src==0x580)
				//      int a=1;
					/*
...
//  printf("killbld prot w\n");
	...
	if(offset==1)
	{
		if(kb_cmd==1)
			...
			res=(0x89911417>>(8*(ptr-1)))&0xff;
	}
//  ErrorLogMessage("%06X: ASIC25 R CMD %X  VAL %X",M68000_regs.pc,kb_cmd,res);
	return res;
}

static DRIVER_INIT( killbld )
{
	UINT16 *PROTDATA = (UINT16*)memory_region(REGION_USER2);
	int i;
	...
	for (i = 0;i < 0x4000/2;i++)
		killbld_sharedprotram[i] = PROTDATA[i];
...
ROM_START( killbld )
	...
	ROM_REGION( 0x4000, REGION_USER2, 0 )
...
ROM_START( kov2 )
	...
	ROM_REGION( 0x1200000, REGION_SOUND1, 0 ) /* Samples - (8 bit mono 11025Hz) - */
	ROM_LOAD( "pgm_m01s.rom", 0x000000, 0x200000, CRC(45ae7159) SHA1(d3ed3ff3464557fd0df6b069b2e431528b0ebfa8) ) // (BIOS)
	ROM_LOAD( "m1200.rom",    0x200000, 0x800000, CRC(b0d88720) SHA1(44ab137e3f8e15b7cb5697ffbd9b1143d8210c4f) )
ROM_END
...
ROM_START( martmast )
	...
	ROM_REGION( 0x1200000, REGION_SOUND1, 0 ) /* Samples - (8 bit mono 11025Hz) - */
	ROM_LOAD( "pgm_m01s.rom", 0x000000, 0x200000, CRC(45ae7159) SHA1(d3ed3ff3464557fd0df6b069b2e431528b0ebfa8) ) // (BIOS)
	ROM_LOAD( "m1000.u5",    0x200000, 0x800000, CRC(ed407ae8) SHA1(a6e9c09b39c13e8fb7fbc89fa9f823cbeb66e901) )
	ROM_LOAD( "m1001.u7",    0xa00000, 0x400000, CRC(662d2d48) SHA1(2fcc3099d9c04456cae3b13035fb28eaf709e7d8) )
ROM_END
...
ROM_START( dmnfrnt )
	...
	ROM_LOAD( "pgm_m01s.rom", 0x000000, 0x200000, CRC(45ae7159) SHA1(d3ed3ff3464557fd0df6b069b2e431528b0ebfa8) ) // (BIOS)
	ROM_LOAD( "w04501.u5",    0x200000, 0x800000, CRC(3ab58137) SHA1(b221f7e551ff0bfa3fd97b6ebedbac69442a66e9) )
ROM_END
...
ROM_START( theglad )
	...
	ROM_REGION( 0xc00000, REGION_SOUND1, 0 ) /* Samples - (8 bit mono 11025Hz) - */
	ROM_LOAD( "pgm_m01s.rom", 0x000000, 0x200000, CRC(45ae7159) SHA1(d3ed3ff3464557fd0df6b069b2e431528b0ebfa8) ) // (BIOS)
	ROM_LOAD( "w04601.u1",    0x200000, 0x800000, CRC(5f15ddb3) SHA1(c38dcef8e06802a84e42a7fc9fa505475fc3ac65) )
ROM_END
...
GAME( 1998, killbld,  pgm,     killbld,sango,    killbld,    ROT0,   "IGS", "The Killing Blade", GAME_IMPERFECT_GRAPHICS | GAME_IMPERFECT_SOUND | GAME_UNEMULATED_PROTECTION | GAME_NOT_WORKING )
GAME( 1998, killbldt, killbld, killbld,sango,    killbld,    ROT0,   "IGS", "The Killing Blade (Taiwan?)", GAME_IMPERFECT_GRAPHICS | GAME_IMPERFECT_SOUND | GAME_UNEMULATED_PROTECTION | GAME_NOT_WORKING ) // one stage doesn't work.. needs better protection sim?
...
GAME( 2001, martmast, pgm,        pgm, sango,    pgm,        ROT0,   "IGS", "Martial Masters", GAME_NOT_WORKING )
GAME( 2002, dmnfrnt,  pgm,        pgm, sango,    pgm,        ROT0,   "IGS", "Demon Front (V102)", GAME_NOT_WORKING )
GAME( 2002, dmnfrnta, dmnfrnt,    pgm, sango,    pgm,        ROT0,   "IGS", "Demon Front (V105)", GAME_NOT_WORKING )
