...
extern int fortyl_vh_start( void );
extern void fortyl_vh_stop( void );
extern void fortyl_vh_screenrefresh( struct mame_bitmap *bitmap, int fullrefresh );
extern void fortyl_convert_color_prom(unsigned char *palette, unsigned short *colortable,const unsigned char *color_prom);
...
static void init_undoukai(void)
{
	from_mcu = 0xff;
	mcu_cmd = -1;
...
static struct MachineDriver machine_driver_40love =
{
	{
		{
			CPU_Z80,
		...
		},
		{
			...
			interrupt, 2	/* source/number of IRQs is unknown */
		},
		{
			CPU_M68705,
			18432000/6, /* OK */
	...
	100,	/* high interleave to ensure proper synchronization of CPUs */
	init_ta7630,	/* init machine */

	/* video hardware */
	64*8, 32*8, { 128,128+255, 2*8, 30*8-1 },
	gfxdecodeinfo,
	1024,0,
	fortyl_convert_color_prom,

	VIDEO_TYPE_RASTER,
	0,
	fortyl_vh_start,
	fortyl_vh_stop,
	fortyl_vh_screenrefresh,
...
static struct MachineDriver machine_driver_undoukai =
{
	{
		{
			CPU_Z80,
			8000000/2,
			undoukai_readmem, undoukai_writemem, 0, 0,
			interrupt, 1
		},
		{
			CPU_Z80 | CPU_AUDIO_CPU,
...
//		}
	...
	60, DEFAULT_REAL_60HZ_VBLANK_DURATION,
	1,	/* high interleave to ensure proper synchronization of CPUs */
	init_ta7630,	/* init machine */
...
ROM_START( 40love )
	...
	ROM_LOAD( "a30-19.ic1", 0x00000, 0x2000, 0x7baca598 )
	ROM_LOAD( "a30-20.ic2", 0x02000, 0x2000, 0xa7b4f2cc )
	ROM_LOAD( "a30-21.ic3", 0x04000, 0x2000, 0x49a372e8 )
	ROM_LOAD( "a30-22.ic4", 0x06000, 0x2000, 0x0c06d2b3 )
	ROM_LOAD( "a30-23.ic5", 0x10000, 0x2000, 0x6dcd186e )	/* banked at 0xa000 */
	ROM_LOAD( "a30-24.ic6", 0x12000, 0x2000, 0x590c20c8 )	/* banked at 0xa000 */

	ROM_REGION( 0x10000, REGION_CPU2, 0 ) /* Z80 sound CPU */
	ROM_LOAD( "a30-08.u08", 0x0000, 0x2000, 0x2fc42ee1 )
	ROM_LOAD( "a30-09.u09", 0x2000, 0x2000, 0x3a75abce )
	ROM_LOAD( "a30-10.u10", 0x4000, 0x2000, 0x393c4b5b )
	ROM_LOAD( "a30-11.u37", 0x6000, 0x2000, 0x11b2c6d2 )
	ROM_LOAD( "a30-12.u38", 0x8000, 0x2000, 0xf7afd475 )
	ROM_LOAD( "a30-13.u39", 0xa000, 0x2000, 0xe806630f )

	ROM_REGION( 0x0800, REGION_CPU3, 0 )	/* 2k for the microcontroller */
	ROM_LOAD( "a30-14"    , 0x0000, 0x0800, 0xc4690279 )

	ROM_REGION( 0x8000, REGION_GFX1, ROMREGION_DISPOSE )
	ROM_LOAD( "a30-25.u22", 0x0000, 0x2000, 0x15e594cf )
	ROM_LOAD( "415f.26", 0x2000, 0x2000, BADCRC( 0x3a45a205 ) )	/* this actually seems good, but we need to find another one to verify */
	ROM_LOAD( "a30-27.u24", 0x4000, 0x2000, 0x57c67f6f )
	ROM_LOAD( "a30-28.u25", 0x6000, 0x2000, 0xd581d067 )

	ROM_REGION( 0x8000, REGION_GFX2, ROMREGION_DISPOSE )
	ROM_LOAD( "a30-29.u62", 0x0000, 0x2000, 0x02deaf40 )
	ROM_LOAD( "a30-30.u63", 0x2000, 0x2000, 0x439f3731 )
	ROM_LOAD( "a30-31.u64", 0x4000, 0x2000, 0x7ed70e81 )
	ROM_LOAD( "a30-32.u65", 0x6000, 0x2000, 0x0434655b )

	ROM_REGION( 0x1000, REGION_PROMS, 0 )
	ROM_LOAD( "a30-15.u03", 0x0000, 0x0400, 0x55e38cc7 )	/* red */
	ROM_LOAD( "a30-16.u01", 0x0400, 0x0400, 0x13997e20 )	/* green */
	ROM_LOAD( "a30-17.u02", 0x0800, 0x0400, 0x5031f2f3 )	/* blue */
	ROM_LOAD( "a30-18.u13", 0x0c00, 0x0400, 0x78697c0f )	/* ??? */

ROM_END

ROM_START( undoukai )
	ROM_REGION( 0x14000, REGION_CPU1, 0 ) /* Z80 main CPU  */
	ROM_LOAD( "a17-01.70c", 0x00000, 0x4000, 0x6ce324d9 )
	ROM_LOAD( "a17-02.71c", 0x04000, 0x4000, 0x055c7ef1 )
	ROM_LOAD( "a17-03.72c", 0x10000, 0x4000, 0x9034a5c5 ) /* banked at 0x8000 */
	ROM_COPY( REGION_CPU1 , 0x10000, 0x8000, 0x2000 ) /* to avoid 'bank bug' */

	ROM_REGION( 0x10000, REGION_CPU2, 0 ) /* Z80 sound CPU */
	ROM_LOAD( "a17-08.8s",  0x0000, 0x2000, 0x2545aa0e )
	ROM_LOAD( "a17-09.9s",  0x2000, 0x2000, 0x57e2cdbb )
	ROM_LOAD( "a17-10.10s", 0x4000, 0x2000, 0x38a288fe )
	ROM_LOAD( "a17-11.37s", 0x6000, 0x2000, 0x036d6969 )
	ROM_LOAD( "a17-12.38s", 0x8000, 0x2000, 0xcb7e6dcd )
	ROM_LOAD( "a17-13.39s", 0xa000, 0x2000, 0x0a40930e )

	ROM_REGION( 0x0800, REGION_CPU3, 0 )	/* 2k for the microcontroller */
	ROM_LOAD( "a17-14.41c", 0x0000, 0x0800, 0x00000000 )

	ROM_REGION( 0x8000, REGION_GFX1, ROMREGION_DISPOSE )
	ROM_LOAD( "a17-04.18v", 0x0000, 0x4000, 0x84dabee2 )
	ROM_LOAD( "a17-05.19v", 0x4000, 0x4000, 0x10bf3451 )

	ROM_REGION( 0x8000, REGION_GFX2, ROMREGION_DISPOSE )
	ROM_LOAD( "a17-06.59v", 0x0000, 0x4000, 0x50f28ad9 )
	ROM_LOAD( "a17-07.60v", 0x4000, 0x4000, 0x7a4b4238 )

	ROM_REGION( 0x1000, REGION_PROMS, 0 )
	ROM_LOAD( "a17-15.10v", 0x0000, 0x0400, 0x9df472b7 )	/* red */
	ROM_LOAD( "a17-16.8v",  0x0400, 0x0400, 0x3bf1ff5f )	/* green */
	ROM_LOAD( "a17-17.9v",  0x0800, 0x0400, 0xc42ae956 )	/* blue */
	ROM_LOAD( "a17-18.23v", 0x0c00, 0x0400, 0x3023a1da )	/* ??? */
