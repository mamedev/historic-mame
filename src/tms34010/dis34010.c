...
{
 int i,j,n;
 char *filename=NULL,buf[34];
 unsigned char *filebuf;
 FILE *f;
 unsigned begin=0,end=(unsigned)-1,offset=0,filelen,len,pc;
 printf ("DIS34010\n"
         "Copyright (C) Zsolt Vasvari/Aaron Giles 1998\n");
  ...
  else
  {
   ...
   switch (j)
   {
    case 0:  ++i; if (i>argc) usage();
             begin=strtoul(argv[i],NULL,16) >> 3;
             break;
    case 1:  ++i; if (i>argc) usage();
             end=strtoul(argv[i],NULL,16) >> 3;
             break;
    case 2:  ++i; if (i>argc) usage();
             offset=strtoul(argv[i],NULL,16) >> 3;
             break;
    default: usage();
   }
  }
 ...
 while (pc<len-1)
 {
  i=(Dasm34010 (filebuf+pc,buf,(pc+offset)<<3))>>3;
  for (j=strlen(buf);j<33;++j) buf[j]=' ';
  buf[33]='\0';
  printf ("    %s    ; %08X ",buf,(pc+offset) << 3);
